  ...
  try {
    ...
    if (req.headers.filter) {
      ...
      if (operator == "substring") {
        ...
        books = await Book.find({ [field]: { [`$${operator}`]: value } });
        if (!books) {
          return res.status(404).json({ message: "Books not found" });
        }
      }
      ...
      if (!paramsBooks) {
        return res
          .status(404)
          .json({ message: `Books not found with the params: ${req.query}` });
      }
    ...
    return res
      .status(500)
      .json({ message: "Error while getting the books. Try again :" + error });
  }
};
