...
export const verifySignUp = (req, res, next) => {
  ...
  if (!username || typeof username !== "string") {
    return res
      .status(400)
      .json({ message: "Username is required and must be string" });
  }
  if (!email || typeof email !== "string") {
    return res
      .status(400)
      .json({ message: "Email is required and must be string" });
  }
  if (!password || typeof password !== "string") {
    return res
      .status(400)
      .json({ message: "Password is required and must be string" });
  }
  next();
};
...
export const verifyDuplicated = async (req, res, next) => {
  ...
  if (userEmail) {
    return res
      .status(400)
      .json({ message: "Email already exists" + userEmail });
  }
  next();
};
...
export const verifyData = (req, res, next) => {
  if (req.body) {
    ...
    if (!title || typeof title !== "string") {
      return res
        .status(400)
        .json({ message: "Title is required and must be string" });
    }
    if (!author || typeof author !== "string") {
      return res
        .status(400)
        .json({ message: "Author is required and must be string" });
    }
    if (!year || typeof year !== "number") {
      return res
        .status(400)
        .json({ message: "Year is required and must be number" });
    }
    if (!pages || typeof pages !== "number") {
      return res
        .status(400)
        .json({ message: "Pages is required and must be number" });
    }
    if (!description || typeof description !== "string") {
      return res
        .status(400)
        .json({ message: "Description is required and must be string" });
    }
    if (!image || typeof image !== "string") {
      return res
        .status(400)
        .json({ message: "Image is required and must be string" });
    }
    next();
  } else {
